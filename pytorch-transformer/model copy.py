...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        tensor_2i = torch.arange(0, d_model, 2).float()
        # (1/10000) ** (2i / d_model) = exp{ -log(10000) * (2i / d_model) }
        div_term = torch.exp(-math.log(10000.0) / d_model * tensor_2i)  # (d_model / 2)
        pe[:, 0::2] = torch.sin(position * div_term)  # sin{ position * [(1/10000) ** (2i / d_model)] }
        pe[:, 1::2] = torch.cos(position * div_term)  # cos{ position * [(1/10000) ** (2i / d_model)] }
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        x = self.norm1(x)
        x = self.self_attention_block(x, x, x, src_mask)
        x = self.dropout(x)
        x = x + x
